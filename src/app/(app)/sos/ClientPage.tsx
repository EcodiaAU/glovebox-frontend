...
async function getPositionNative(timeoutMs = 120_000): Promise<GeoResult> {
  ...
  try {
    if (Capacitor.isNativePlatform()) {
      ...
      if (!isLocationGranted(perms)) {
        throw new Error("Location permission denied. Please allow it in Settings → Nav. → Location.");
      }
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--c-text-muted)" }}>
              Nav keeps your last known position even without signal. SMS works on any one bar of 2G.
            </div>
          </div>
        </div>
      </div>
