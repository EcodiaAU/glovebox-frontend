...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInWithGoogle = useCallback(async () => {
    ...
    const redirectTo = Capacitor.isNativePlatform()
      ? "https://nav.ecodia.au/auth/callback"
      : `${window.location.origin}/auth/callback`;

    if (Capacitor.isNativePlatform()) {
      // skipBrowserRedirect prevents Supabase from calling window.location.href,
      // which would navigate the WebView off nav.ecodia.au into Safari.
  ...
  const signInWithAppleNative = useCallback(async () => {
    try {
      ...
      const result = await SignInWithApple.authorize({
        ...
        redirectURI: "https://nav.ecodia.au/auth/callback", // unused on native, required by plugin types
        scopes: "email name",
        state: `roam-${Date.now()}`,
        nonce: nonceHash,
      });
